m = [s.strip() for s in open('day10.txt', 'r').readlines()]  # create matrix
...
head_targets = {}
for head in heads:
    head_targets[head] = set()


def height(r, c):
    return int(m[r][c]) if m[r][c] != '.' else -5


# walk from given position
def walk(r, c, dist, start_pos):
    # height
    h = height(r, c)
    # distance from start
    d = dist + 1
